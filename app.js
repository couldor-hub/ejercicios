...
var todosEjercicios = [
  ...
  // 💪 MANTENER FORMA - próximamente
  // ⚡ GANAR FUERZA - próximamente
];
...
function iniciarEntrenamientoAbdomen() {
  ...
  ejerciciosHoy = mezclar(ejerciciosAbdomen).slice(0, num);
  ejercicioActual = 0;
  iniciarEjercicio(0);
}
...
function iniciarEstiramientos() {
  if (!perfil) { irA('datos'); return; }
  if (ejerciciosEstiramientos.length === 0) {
    alert('Estiramientos próximamente 🧘');
    return;
  }
...
function beep(frecuencia, duracion, volumen) {
  ...
  promise.then(function() {
    ...
    o.type = 'sine';  // ← volvemos a sine, más limpio
    o.frequency.value = frecuencia;
    g.gain.setValueAtTime(1.0, audioCtx.currentTime);
    g.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + duracion); // ← fade out suave
    o.connect(g);
...
function sonidoInicio() {
  beep(523, 0.15, 1.0);  // Do
  setTimeout(function() { beep(659, 0.15, 1.0); }, 180);  // Mi
  setTimeout(function() { beep(784, 0.3,  1.0); }, 360);  // Sol
}

function sonidoCuenta() { beep(880, 0.12, 1.0); }   // La agudo, corto
function sonidoFin()    {
  beep(784, 0.15, 1.0);
...
// Y en iniciarEntrenamiento():
ejerciciosHoy = mezclar(todosEjercicios).slice(0, num);
...
function siguienteEjercicio() {
  ...
  musica.currentTime = 0;
  if (ejercicioActual + 1 < ejerciciosHoy.length) {
    iniciarEjercicio(ejercicioActual + 1);  // ← iniciarEjercicio ya pone entrenamientoActivo = true
  } else {
    finalizarEntrenamiento();
  }
}
